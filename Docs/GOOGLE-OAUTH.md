...
- **Password login** remains available for the same accounts.
...
2. Put secrets in **`Backend/.env`** (copy from `Backend/.env.example`) or in repo-root **`deploy.env`**. The server loads **`deploy.env` first**, then **`Backend/.env`**, without overwriting keys already set — so production can keep using only `deploy.env` on the server.
